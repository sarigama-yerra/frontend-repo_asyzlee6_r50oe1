  ...
  return (
    <div className="py-12">
      <header className="max-w-3xl mt-10">
        ...
        <p className="mt-3 text-slate-700">I help brands ship fast, beautiful websites with motion and meaning.</p>
      </header>
      <div className="grid md:grid-cols-2 gap-8 mt-10">
        {sections.map((s, i)=> (
          <motion.section key={s.title} initial={{opacity:0, y: 8}} whileInView={{opacity:1, y:0}} viewport={{ once: true }} transition={{delay: i*0.05}} className="bg-white rounded-xl p-6 border shadow-sm">
            <h3 className="font-semibold text-lg">{s.title}</h3>
            <p className="text-slate-700 mt-2 text-sm leading-6">{s.body}</p>
          </motion.section>
        ))}
      </div>

      <section className="mt-12 bg-gradient-to-r from-indigo-50 to-blue-50 border rounded-xl p-8">
        <h3 className="font-semibold text-lg">Experience</h3>
        <ul className="mt-4 grid md:grid-cols-3 gap-4 text-sm text-slate-700">
          <li className="bg-white border rounded-lg p-4">Frontend Developer — 5+ years building high-perf UIs</li>
          <li className="bg-white border rounded-lg p-4">WordPress Expert — Elementor & WooCommerce</li>
          <li className="bg-white border rounded-lg p-4">Product Design — systems, tokens, motion</li>
        </ul>
      </section>
    </div>
  );
